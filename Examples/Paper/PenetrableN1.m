...
% bugs and problems to the public mailing list <mu-diff@math.cnrs.fr>.
% 
% ~~~
%
...
switch GEOMETRY
    ...
    case 'rectangular',        
        ...
        centerx = -(Nx-1)*bx/2;
        centery = -(Ny-1)*by/2;
        O = RectangularLattice(bx, by, Nx, Ny, 'Center', [0, 0]);
        a = ones(size(O,2));
    case 'triangular',
        bx = 1; by = 1;
        Nx = 2; Ny = 2;
...
mu0      = 1/omega;
%Obstacles:
%epsilon_m = (0.7+0.8*rand(1,N_scat)).^2*epsilon0;
epsilon_m = 2^2*epsilon0;
mu_m      = mu0;
...
%% Display on mesh
clear XX YY Ue Ui U UincOnMesh A rho rho_plus rho_minus;
save('dataN1.mat');